export const meta = {
  name: 'build-loop',
  description:
    'The shipped legion build stage, MILESTONE-INTERLEAVED: for each milestone of the approved, hash-locked plan — every outstanding task (brief, build, VERIFIED gate receipt, review at the task\'s risk tier, one fix round, recorded verdicts, typed task-done), then that milestone CLOSES (squash, boundary gate, milestone code review, product review) before the next milestone starts. Fail-closed, re-runnable: done tasks skip and a milestone whose close verdicts are already recorded passing skips its close.',
  whenToUse:
  ...
  phases: [
    { title: 'Milestone tasks', detail: 'per task: builder, kernel-verified gate receipt, review at the task risk tier, at most one fix round, typed task-done' },
...
// build round once re-reviewed a CODEX fail with the CLAUDE lens: the fix was blessed by an agent that
...
// codex-consult is DELIBERATELY absent from this set (operator ruling 2026-07-31: the consult is
// a second lens, never the unique one). On full the close still DISPATCHES it — see the advisory
...
const deferred = []
const degraded = []
// THE CODEX LATCH. A dispatch that reports the lens dead still COSTS a dispatch — measured at
// 26 415 tokens for one quota answer, because every turn of a subagent re-bills its context and
// there is no cheap way to abandon one. The loop used to have no memory of that answer: a
// ten-task feature paid ten times for it. So the first DURABLE absence latches the lens off for
// the rest of the run — no second dispatch, and every review from here still returned as
// `degraded`, which is the half that must not be bought away.
// Durable is a closed list, and the two exclusions are deliberate, both toward not silently
// losing a lens: `other` (unknown cause) does NOT latch — pay rather than skip — and a transient
// cause never latches however often it repeats (no consecutive-failure counter). A result with no
// `unavailable` field at all — an older plugin build of the agent — latches nothing and behaves
...
/** Latch the codex lens off when THIS result says its absence is durable. Called with every codex
 * result the loop sees — task review and milestone close alike — and idempotent: the first
 * durable absence wins, so `after` names the subject that actually paid for the discovery and the
 * log line is written once, not once per later task.
 * `reason` is the CLASSIFIED cause (the enum), `detail` codex's own message — the review artifact
 * quotes the second and decides on the first. */
...
for (const group of groups) {
  ...
  for (const task of group.outstanding) {
    ...
    // 'low' (docs-only, test-only, low blast radius): ONE lens — legion:code-reviewer. Codex is
    ...
    let claudeResults = []
    let codexLens = null
    // Read BEFORE the dispatch, because THIS task's own result may set the latch below: it is what
    // tells "never dispatched, the lens was already dark" from "dispatched and came back dead" —
    // the same degradation either way, but only one of them was paid for here.
    ...
    if (dual) {
      const lenses = await parallel([
        ...
        // THE LATCHED LENS IS NOT DISPATCHED AT ALL (see codexOff): a subagent bills its context on
        // every turn, so the only dispatch that costs nothing is the one that never happens.
      ...
      if (!codexLens || codexLens.available === false) {
        ...
        // THE LATCH BUYS THE DISPATCH AWAY, NEVER THE DEGRADATION: skipping the thunk leaves
        // `codexLens` falsy, which lands here exactly as an unavailable lens does.
...
async function closeMilestone(group) {
  ...
  if (!SQUASH) {
    ...
    const sqRes = await agent(
      [
        ...
        '',
        'THE REWRITE WINDOW IS EXACTLY THIS MILESTONE. It starts after the commit that closed the',
        'PREVIOUS milestone (a squashed conventional commit produced by this same step) or, for the',
        "first milestone, after the feature's pinned base. You may NOT rewrite at or before that",
        'boundary: those commits carry another milestone\'s certified history. If you cannot',
        'identify the boundary with certainty, return status "refused" and say why — a rebase that',
        'reaches too far is not recoverable by anything downstream.',
        '',
  ...
  // ever counts). The check runs BEFORE the advisory codex push below, deliberately: the consult
  ...
  // The ADVISORY consult lens (full only; operator ruling 2026-07-31: a second lens, never the
  // unique one). Dispatched and recorded when it runs; null / available:false degrades on record
  // — never a verdict, never a failed close, never counted by the resume predicate.
  // `closeDegraded` is declared HERE, above the dispatch loop that normally fills it, because THE
  // LATCH DEGRADES WITHOUT A DISPATCH: a role never pushed into `roles` has no `results[i]` to
  // reach that loop, and a close that skipped its second lens silently would read as a full-profile
  // close that got one. At task scope the same skip degrades for free (the falsy `codexLens`
  // branch); here it has to be pushed by hand, and that asymmetry is the whole risk in the latch.
  const closeDegraded = []
  if (PROFILE === 'full') {
    if (codexOff) {
  ...
  // second lens, never the unique one): codex returning nothing or available:false is the
  ...
  roles.forEach((r, i) => {
    const raw = results[i]
    if (r.role === 'codex-consult' && (!raw || raw.available === false)) {
  ...
  if (failing.length > 0) {
    ...
    for (const run of failing) {
      ...
      if (!reReview || reReview.available === false) {
        ...
        // forged evidence — and fail the close. This holds for the advisory codex lens too, and
    ...
    for (const run of runs.filter(r => !failing.includes(r))) {
      ...
      if (!reCert || reCert.available === false) {
        ...
        // set, not the resume predicate — so a vanished codex costs the evidence a degradation
...
function closeReviewPrompt(group, role, ids) {
  ...
  if (role === 'codex-consult') {
    return `${head}` +
      `Second-opinion CONSULT at MILESTONE scope: assemble this milestone's ASSEMBLED diff — the ` +
      `squashed milestone commit (plus its close-fix commit if the log shows one), not one ` +
      `task's — and hand the external codex CLI that diff with the milestone-mode question: the ` +
      `seams between the tasks, the interfaces they agreed on, and anything only wrong when the ` +
      `tasks are read together. The per-task consults already happened; do not repeat them. ` +
      `Return codex's findings verbatim in substance. If the CLI is unavailable, return ` +
  ...
  return `${head}` +
    ...
    `only wrong when the tasks are read together. The per-task reviews already happened; do not ` +
    `repeat them.\n${BLAST_RADIUS}`
}
...
return {
  ...
  deferred,
  degraded,
  // null, or {after, reason, detail} — the subject that discovered the durable absence, the
  // classified cause and codex's own message. `degraded` says WHICH reviews lost the lens; this
  // says when it went dark for good and why, which is the half the review artifact can quote.
  codexOff,
  singleLens,
  tiersIgnored,
