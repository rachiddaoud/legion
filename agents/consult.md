...
**Independence guard.** On any API backend, a `consult_model` matching `/claude/i` is
`misconfigured` and you stop. The lens exists to be a second, **non-Claude** opinion; a Claude
model here buys the same blind spots twice at the price of an extra dispatch.
...
3. **Assemble the diff yourself and pipe it in.** Gemini has no review subcommand and no scope
   ...
   below, which is what makes one translation table serve all three backends:
...
5. **Translate field by field.** The mapping is fixed so that no tiering judgement is ever yours —
   and it is ONE table for all three backends, because the pinned prompts of 3b and 3c define the
   same priority polarity that codex's own CLI produces:
...
6. **`unavailable` names WHICH absence, by LOOKUP — never by judgement.** The caller latches the
   ...
   | the run hit its bound (the perl alarm, or curl's `--max-time`) | `timeout` |
...
```json
{
  ...
  "backend": "codex|gemini|openai|google|xai|deepseek|mistral|api",
